...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    if self.cell_type == 'rnn':
        dword_vectors, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, rnn_cache)
    else:
        dword_vectors, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, lstm_cache)
  ...
  def sample(self, features, max_length=30):
    ...
    T = max_length
    prev_word = np.repeat(self._start, N)#(N, )  
    for i in range(T):
        word_vector = W_embed[prev_word]#(N, W)
        if self.cell_type == 'rnn':
            next_h, cache = rnn_step_forward(word_vector, prev_h, Wx, Wh, b)
            prev_h = next_h
        else:
            next_h, next_c, cache = lstm_step_forward(word_vector, prev_h, prev_c, Wx, Wh, b)
            prev_h = next_h
            prev_c = next_c        
        scores = next_h.dot(W_vocab) + b_vocab#(N, V)
        prev_word = np.argmax(scores, axis = 1)#(N, )
